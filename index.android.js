...
var sidebarWidth = (deviceWidth*2)/3
...
class rubyChina extends Component {
  ...
  componentWillMount() {
    this.state.panResponder = PanResponder.create({
      ...
      onPanResponderMove: (e, gestureState)=>{
        if(gestureState.dx > 0 && this.state.sidebarHidden){
          this.state.pan.setValue({x:gestureState.dx, y:0})
        }
        if(gestureState.dx < 0 && !this.state.sidebarHidden){
          this.state.pan.setValue({x:(sidebarWidth+gestureState.dx), y: 0})
        }
      },
      onPanResponderRelease: (e, gestureState) => {
        if(this.state.sidebarHidden && gestureState.dx > 0){
          if(gestureState.moveX > sidebarWidth/3){
            this.setState({
              sidebarHidden: false
            });
            Animated.spring(
              this.state.pan,
              {toValue: {x: sidebarWidth, y: 0}, bounciness: 0}
            ).start()
          } else {
            Animated.spring(
              this.state.pan,
              {toValue: {x: 0, y: 0}, bounciness: 0}
            ).start()
          }
        }
  ...
  render(){
    this.state.pan.x.interpolate({
      inputRange: [0, sidebarWidth/3],
      outputRange: [0, sidebarWidth],
      extrapolate: 'clamp'
    });

    var topics = MOCK_TOPICS.topics.map((item, index)=>{
    ...
    return (
      <View style={styles.container} {...this.state.panResponder.panHandlers}>
        <ToolbarAndroid
            ref={component => this._toolbar = component}
            actions={[{title:'a', show: 'always'}]}
            style={[styles.toolbar, {height: (this.state.toolbarHidden ? 0 : 56)}]} />
        <ScrollView onScroll={this.handleScroll.bind(this)}
              scrollEventThrottle={200}>
          {topics}
        </ScrollView>
        <Sidebar size={{width: sidebarWidth, height: deviceHeight }} right={deviceWidth} pan={this.state.pan} />
      </View>
    );
  }
}
